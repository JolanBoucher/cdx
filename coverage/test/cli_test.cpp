...
// CliArgs / parse_args / QuerySelection / etc. now live in namespace
// cdx_coverage (see cli.hpp).
using namespace cdx_coverage;
...
namespace {
    ...
    TEST(CliBasicArgsTest, MissingGamFileImpliesInspectMode) {
        const CliArgs args = parseWithCdxOnly();
...
// =============================================================================
// Inspect mode (implicit: no GAM positional given). There is no -i/--inspect
// flag anymore - see cli.cpp. The target component, if any, comes from
// -q/--query.
// =============================================================================
namespace {
    TEST(CliInspectOptionTest, NoGamAloneEnablesWithNoComponentSelected) {
        const CliArgs args = parseWithCdxOnly();
        EXPECT_TRUE(args.inspectMode());
        EXPECT_FALSE(args.inspect.component.has_value());
    }

    TEST(CliInspectOptionTest, QueryFlagSelectsComponentToInspect) {
        const CliArgs args = parseWithCdxOnly({"-q", "chr1"});
        EXPECT_TRUE(args.inspectMode());
        ASSERT_TRUE(args.inspect.component.has_value());
        EXPECT_EQ(*args.inspect.component, "chr1");
        // The range is meaningless in inspect mode: args.query is cleared
        // once its component has been captured into args.inspect.component.
        EXPECT_FALSE(args.query.has_value());
    }

    TEST(CliInspectOptionTest, QueryRangeIsIgnoredInInspectMode) {
        const CliArgs args = parseWithCdxOnly({"-q", "chr1 1000:5000"});
        EXPECT_TRUE(args.inspectMode());
        ASSERT_TRUE(args.inspect.component.has_value());
        EXPECT_EQ(*args.inspect.component, "chr1");
        EXPECT_FALSE(args.query.has_value());
    ...
    TEST(CliInspectOptionTest, AbsentWhenGamIsProvided) {
