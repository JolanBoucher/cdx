...
TEST(MainE2eInspectModeTest, SummaryListsAllComponentsAndSkipsOutputDirectory) {
    ...
    // Inspect mode is implicit: a lone CDX positional (no GAM) triggers it -
    // there is no -i/--inspect flag anymore (see cli.cpp).
    const RunResult result = runCdxCoverage({cdx.path().string(), "-o", out_dir.string()});
...
TEST(MainE2eInspectModeTest, SingleComponentByNameShowsOnlyThatComponent) {
    ...
    // The component to inspect is selected via -q/--query (its range, if any,
    // is ignored in inspect mode - see cli.cpp).
    const RunResult result = runCdxCoverage({
        cdx.path().string(), "-q", "chr1", "-o", out_dir.path().string()
    });
...
TEST(MainE2eInspectModeTest, InspectModeDoesNotRequireAGamFile) {
    ...
    // No GAM path at all - this is precisely what triggers inspect mode.
