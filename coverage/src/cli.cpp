...
CliArgs parse_args(const int argc, char **argv) {
    ...
    //
    // Inspection mode is implicit: a lone CDX file (no GAM) means "inspect".
    // The optional target component comes from -q/--query (parsed below);
    // its range, if any, is meaningless in inspect mode and is simply
    // ignored - only the component identifier is kept.
    if (args.gam_file.empty()) {
    ...
    if (!query_raw.empty()) {
        ...
        // In inspect mode, -q/--query only ever selects which single
        // component to display; any range it carries is meaningless there
        // (inspect has no coverage to trim) and is intentionally dropped.
        if (args.inspectMode()) {
            args.inspect.component = args.query->component;
            args.query.reset();
        }
    }
