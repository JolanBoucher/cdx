/**
 * @file coverage_app.cpp
 * @brief Implementation of the CDX coverage application (cdx_coverage::run).
 *
 ...
 *   - Inspect indexed components and metadata (inspection mode - implicit
 *     whenever no GAM file is supplied, see cli.cpp).
...
// CliArgs / parse_args / QuerySelection / ComponentType / InspectOptions now
// live in namespace cdx_coverage (see cli.hpp) so they don't collide at
// link time with builder's identically-named symbols in the merged cdx
// executable.
...
int cdx_coverage::run(int argc, char **argv) {
