...
// Everything below lives in namespace cdx_builder specifically so that
// `CliArgs`/`parse_args`/`prepare_output_filepath` cannot collide with the
// identically-named (but semantically different) declarations in
// coverage/src/cli.hpp: both cdx_builder_core and cdx_coverage_core get
// statically linked into the single merged "cdx" executable (see
// ../../CMakeLists.txt), and without this namespace, un-namespaced global
// symbols with the same signature in two different static libraries silently
// collide at link time - the linker keeps whichever definition it pulls in
// first and simply never links the other object file in, no error, no
// warning. That's a real bug this project hit: the coverage branch ended up
// silently calling the builder branch's parse_args().
