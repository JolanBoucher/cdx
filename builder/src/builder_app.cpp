/**
 * @file builder_app.cpp
 * @brief Implementation of the CDX index-building pipeline (cdx_builder::run).
 *
 * This file contains the program logic previously found directly in main():
 * parsing command-line arguments, managing top-level execution flow, catching
 * runtime errors, and invoking the CDX graph processing pipeline
 * (`run_pipeline`).
 *
 * @details
...
// CliArgs / parse_args / prepare_output_filepath now live in namespace
// cdx_builder (see cli.hpp) so they don't collide at link time with
// coverage's identically-named symbols in the merged cdx executable.
using namespace cdx_builder;
 ...
 */
void run_pipeline(const CliArgs& args) {
    using Clock = std::chrono::steady_clock;
...
int cdx_builder::run(const int argc, char** argv) {
    try {
