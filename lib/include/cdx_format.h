...
namespace cdx {
     ...
     *
     *     [ComponentHeader]
     *     [ComponentName (name_size bytes)]
     *     [NodeRecord × n_records]
     */
#pragma pack(push, 1)
    /**
    ...
    struct ComponentHeader {
        RecordCount n_records; // Total number of node records belonging to this component
        Nid node_id_min; // Minimum node identifier value present in this component block
        Nid node_id_max; // Maximum node identifier value present in this component block
     ...
     */
    struct NodeRecord {
        Nid node_id; // Unique global identifier of the graph node
        Idx idx; // Assigned local sequential rank or layout index within the component
        SeqLen seq_len; // Biological sequence length associated with the node
    };
#pragma  pack(pop)

    // Static assertion checks to guarantee exact binary layout size
    static_assert(sizeof(FileHeader) == 10, "CdxFileHeader must occupy exactly 10 bytes");
    static_assert(sizeof(ComponentHeader) == 28, "CdxComponentHeader must occupy exactly 28 bytes");
    static_assert(sizeof(NodeRecord) == 16, "CdxNodeRecord must occupy exactly 16 bytes");

    static_assert(std::is_trivially_copyable_v<FileHeader>, "CdxFileHeader must be trivially copyable");
    static_assert(std::is_trivially_copyable_v<ComponentHeader>, "CdxComponentHeader must be trivially copyable");
    static_assert(std::is_trivially_copyable_v<NodeRecord>, "CdxNodeRecord must be trivially copyable");
    ...
    public:
        ...
        // Magic signature bytes identifying valid CDX files
        ...
        static constexpr std::size_t FILE_HEADER_SIZE = sizeof(FileHeader);
        // Total byte size of the global file header (10 bytes)
        static constexpr std::size_t COMPONENT_HEADER_SIZE = sizeof(ComponentHeader);
        // Total byte size of each component block header (28 bytes)
        static constexpr std::size_t RECORD_SIZE = sizeof(NodeRecord);
        // Total byte size of an individual node record (16 bytes)
...
#error "Unable to determine native byte order for the CDX format"
        ...
        static void convert_node_records_to_native(
            NodeRecord *records,
            std::size_t count
        ) noexcept;
        ...
        static void pack_node_record(
            ...
            Nid node_id,
            Idx local_index,
            SeqLen sequence_length) noexcept;
         ...
         */
        [[nodiscard]] static FileHeader unpack_file_header(const char *source) noexcept;
         ...
         */
        [[nodiscard]] static ComponentHeader unpack_component_header(const char *source) noexcept;
         ...
         */
        [[nodiscard]] static NodeRecord unpack_node_record(const char *source) noexcept;

        // --- Validation Helpers ---
        [[nodiscard]] static bool has_valid_magic(const FileHeader &header) noexcept;

        [[nodiscard]] static bool has_valid_widths(const FileHeader &header) noexcept;
    };
} // namespace cdx
