...
namespace cdx {
    ...
    void CdxFormat::convert_node_records_to_native(
        NodeRecord *records,
        const std::size_t count
        ) noexcept {
        if constexpr (!IS_LITTLE_ENDIAN) {
            for (std::size_t i = 0; i < count; ++i) {
                records[i].node_id = from_little_endian64(records[i].node_id);
                records[i].idx = from_little_endian32(records[i].idx);
                records[i].seq_len = from_little_endian32(records[i].seq_len);
            }
        }
    }
    ...
    FileHeader CdxFormat::unpack_file_header(const char *source) noexcept {
        FileHeader header{};
    ...
    ComponentHeader CdxFormat::unpack_component_header(const char *source) noexcept {
        ComponentHeader header{};
        std::memcpy(&header, source, sizeof(header));
    ...
    NodeRecord CdxFormat::unpack_node_record(const char *source) noexcept {
        NodeRecord record{};
    ...
    bool CdxFormat::has_valid_magic(const FileHeader &header) noexcept {
        return header.magic == MAGIC;
    }

    // Validates whether the file header specifies supported node ID and sequence length byte widths.
    bool CdxFormat::has_valid_widths(const FileHeader &header) noexcept {
